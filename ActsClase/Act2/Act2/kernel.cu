...
__global__ void print_all_idx()
{
    ...
    printf("[DEVICE] threadIdx.x %d, blockIdx.x: %d, gridDime.x: %d \n", tidx, bidx, gdimx);
    printf("[DEVICE] threadIdx.y %d, blockIdx.y: %d, gridDime.y: %d \n", tidy, bidy, gdimy);
    printf("[DEVICE] threadIdx.z %d, blockIdx.z: %d, gridDime.z: %d \n", tidz, bidz, gdimz);
}
